...
현재 iOS 포트는 한국어 제스처 키보드, 문장부호 키보드, 숫자 키보드,
전화번호 패드, 이모지 키보드를 포함합니다. 다른 키보드로의 전환은 iOS 방식에 맞춰
지구본 키로 처리하며, 하단 `!#1` 키는 `한글 -> 문장부호 -> 숫자` 순으로 순환합니다.
iOS 커스텀 키보드 제약 안에서 실사용이 가능하도록 구현하는 것을 목표로 하고 있으며,
의도대로 동작하지 않는다면 구현상의 버그입니다. 사용 중 문제가 발생한다면 이슈를 남겨주세요.

다만 iOS의 커스텀 키보드는 플랫폼 제약이 있습니다. 보안 입력 필드에서는 시스템
키보드가 대신 표시될 수 있고, 텍스트 선택과 일부 편집 동작은 호스트 앱 제어
영역이기 때문에 완전히 동일하게 재현하기 어렵습니다.

설치 후에는 iOS 설정 앱에서 다음 항목을 조정할 수 있습니다.
